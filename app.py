...
def process_film_number(message):
    try:
        ...
        if 0 <= film_index < len(films_data):
            ...
            film_details = get_film_details(film, message.chat.id)  # Pass the chat ID
...
def add_to_watch_later(call):
    ...
    button_back_film = types.InlineKeyboardButton("Вернуться к топ фильмам", callback_data="back_to_film_list")
    button_back = types.InlineKeyboardButton("Назад", callback_data="back_to_menu")
    markup = types.InlineKeyboardMarkup()
    markup.add(button_back)
    markup.add(button_back_film)
    bot.send_message(call.message.chat.id, "Выберите действие:", reply_markup=markup)
...
def random_film(message):
    if films_data:
        ...
        bot.send_message(message.chat.id, f"Советую посмотреть: {film['nameRu']} ({film['year']})")
        button_back = types.InlineKeyboardButton("Назад", callback_data="back_to_menu")
...
def search_film_by_keyword(message):
    ...
    if response.status_code == 200:
        ...
        if films:
            search_results = ""
            for i, film in enumerate(films[:10], start=1):
                genres = ", ".join([genre['genre'] for genre in film['genres']])
                countries = ", ".join([country['country'] for country in film['countries']])
            ...
            bot.register_next_step_handler(msg, process_search_film_number, films)
        else:
...
def process_search_film_number(message, films):
    try:
        film_index = int(message.text) - 1
        if 0 <= film_index < len(films):
